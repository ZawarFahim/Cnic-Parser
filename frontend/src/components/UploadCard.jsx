...
export default function UploadCard({ title, subtitle, file, onFileSelect }) {
  ...
  const handleFile = useCallback((selectedFile) => {
    ...
    if (!ACCEPTED.includes(selectedFile.type)) { setError('Unsupported format. Use JPG, PNG, or WebP.'); return }
    if (selectedFile.size > MAX_SIZE_MB * 1024 * 1024) { setError(`Max size is ${MAX_SIZE_MB} MB.`); return }
    onFileSelect?.(selectedFile)
  }, [onFileSelect])

  const onDrop    = useCallback((e) => { e.preventDefault(); setDragOver(false); handleFile(e.dataTransfer.files[0]) }, [handleFile])
  const clearFile = () => { onFileSelect?.(null) }
  ...
  return (
    <div className="card" style={{ display: 'flex', height: '100%', overflow: 'hidden', background: '#141414' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '14px 16px', gap: '10px', borderRight: '1px solid rgba(255,255,255,0.05)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            ...
            <p style={{ fontSize: '0.62rem', color: '#555', marginTop: '2px' }}>{subtitle}</p>
          </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', padding: '16px' }}>
                ...
                <div style={{ textAlign: 'center' }}>
                  <p style={{ fontSize: '0.78rem', fontWeight: 500, color: dragOver ? '#10b981' : '#888' }}>
                    {dragOver ? 'Drop to upload' : 'Drop CNIC side here'}
                  </p>
      ...
      {/* ── Right section: file info + browse ── */}
      <div style={{ width: '140px', flexShrink: 0, display: 'flex', flexDirection: 'column', padding: '14px 14px', gap: '10px', justifyContent: 'space-between' }}>
        <div>
          ...
          {preview && fileName ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              <div>
                <p style={{ fontSize: '0.72rem', fontWeight: 500, color: '#ccc', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '110px' }}>{fileName}</p>
                <p style={{ fontSize: '0.65rem', color: '#555', marginTop: '2px' }}>{fileSize}</p>
              </div>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              {['Clear image', 'Good lighting', 'No cropping'].map(tip => (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          <button className="btn btn-secondary" style={{ width: '100%', fontSize: '0.72rem', padding: '9px 10px' }} onClick={() => inputRef.current?.click()}>
            <Upload size={12} />{preview ? 'Replace' : 'Browse'}
          </button>
        </div>
      </div>
